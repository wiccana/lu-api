...
public interface ItemRepository extends JpaRepository<Item, Integer> {
    ...
    @Query(value = "SELECT item_id, item_number, name, category, supplier_id, description, cost_price, unit_price, history FROM Ospos_items OT where OT.supplier_id = :supplierId", nativeQuery = true)
    List<Item> findBySupplierId(@Param("supplierId") Integer supplierId);

    @Query(value = "SELECT item_id, item_number, name, category, supplier_id, description, cost_price, unit_price, history FROM Ospos_items OT where OT.category = :category%", nativeQuery = true)
